...
const Productlist = () => {
    ...
    {
        let result= await fetch('http://mobile.backend.alphasconsulting.com/products',
        {
          headers:
          {
    ...
    const Delete= async(id)=>   
    {
        ...
        let result = await fetch(`http://mobile.backend.alphasconsulting.com/product/${id}`,{
            method:"Delete",
            headers:
            {
              // this bearer is extra for more secure on BE side I removed this
    ...
    const handelSearch= async(event)=>
    {
        ...
        if(key)
        {
            let result= await fetch(`http://mobile.backend.alphasconsulting.com/search/${key}`,{
              headers:
              {
                // this bearer is extra for more secure on BE side I removed this
                // so if any body genreate token and pass only token not with any other word with space
  ...
  return (
    <div className='product-list' style={{ maxHeight: '100%', overflowY: 'auto' }}>
      ...
      <ul>
        ...
        <li className='listHeader'>IMEI 1</li> 
        <li className='listHeader'>IMEI 2</li>
        <li className='listHeader'>Details</li>
